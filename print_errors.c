...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/06/03 17:57:53 by gkord             #+#    #+#             */
/*   Updated: 2022/06/04 04:44:17 by iwhana21         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	redirect_errors(t_command_and_flag *reverse_head)
{
	if (!reverse_head)
		return ;
	if ((reverse_head->pape == LESS && \
	(reverse_head->f_error == -3 || reverse_head->f_error == -1)) || \
	...
	else if ((reverse_head->pape == LESS && \
	reverse_head->f_error == -2) || ((reverse_head->pape == MORE || \
...
int	builtins_errors(t_command_and_flag *reverse_head, int fd1, t_env *env)
{
...
int size, t_env *env)
{
	...
	{	
		...
		if (builtins_errors(reverse_head, fd1, env) && fd1 != 0)
